...
public final class PlayerDataDataFixer {
    ...
    public static class RemoveEmptyObjectSpawnFix extends DataFix {
        ...
        public TypeRewriteRule makeRule() {
            ...
            return fixTypeEverywhereTyped("NoEmptyHomesObject", inputType, typed -> {
                return typed.update(DSL.remainderFinder(), this::removeEmptyHomesField);
            });
        }

        private Dynamic<?> removeEmptyHomesField(Dynamic<?> dynamic) {
            var hasSpawnValueWithEmptyObject = dynamic.get("homes")
                .get()
                .mapOrElse( // if we've got a homes object
                    spawn -> spawn.getMapValues().map(Map::isEmpty).getOrThrow(), // and that home object is empty
                    (_keyMissingError) -> false // (false if no homes key in PlayerData)
                );
